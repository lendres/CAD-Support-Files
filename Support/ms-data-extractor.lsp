...
(defun C:MS-DATA-EXTRACTOR (/ ACSSTS CMD CNT DWG_NAME FILE_HAND FILTLST SSVL SSXPL 
                            PREFIX TEMP VERS
                           ) 
  (setq OLDERR  *ERROR*
          ...
          (setq TEMPLST (append 
                          ...
                          (list 
                            (cdr 
                              (assoc 
                                2
                                (entget (ssname TEMP CNT)) ;_ End entget
                              ) ;_ End assoc
                            ) ;_ End cdr
                          ) ;_ End list
                        ) ;_ End append
...
(defun MAKE-FONT-LISTS (TEMP) 
  ...
  (if (<= EXTEN 3) 
    (setq EXTEN NIL)
    (setq EXTEN (substr TEMP (- (strlen TEMP) 3))) ;_ End setq
  ) ;_ End if
  (if (/= TEMP "") 
    (if (= EXTEN ".TTF") 
